...
lgd(1) = surf(xgrid(subgrid), xgrid(subgrid), y3(subgrid,subgrid), 'EdgeColor', 'k'); hold('on'); colormap(gray);

plot3(xgrid, zeros(1,numel(y2)), y2, 'r-', 'LineWidth', lw2);
lgd(2) = fill3(xgrid, zeros(1,numel(y2)), y2, 'k', 'EdgeColor', 'None', 'FaceColor', 'r', 'FaceAlpha', 0.5);

plot3(0.5, 0.5, max(y3(:)), 'bo', 'MarkerFaceColor', 'b', 'MarkerSize', 15);
text(0.5, 0.5, max(y3(:)), sprintf('  Mean = %1.5f\n   Max = %1.5f', mean(y3(:)), max(y3(:))), 'HorizontalAlignment', 'Left', 'FontSize', 15, 'Color', 'b');
text(0.5, 0, max(y2(:)), sprintf('  Mean = %1.5f\n   Max = %1.5f', mean(y2(:)), max(y2(:))), 'HorizontalAlignment', 'Left', 'FontSize', 15, 'Color', 'b');
plot3(0.5, 0, max(y2(:)), 'bo', 'MarkerFaceColor', 'b', 'MarkerSize', 15);

axis([0,1,0,1,0,max(y3(:))]);
set(gca, 'XTickLabel', {}, 'YTickLabel', {}, 'ZTickLabel', {}, 'Xgrid', 'Off', 'Ygrid', 'Off', 'FontSize', 15, 'LineWidth', 1);
